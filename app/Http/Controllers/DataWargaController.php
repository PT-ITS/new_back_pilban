...
use App\Exports\DataByKabExport;
...
class DataWargaController extends Controller
{
    public function exportToExcelByKec($id)
    {
        try {
            ...
            // Nama file menggunakan nama kabupaten
            // $kabupatenName = MasterDataWarga::where('kabupaten_id', $id)->value('nama_kabupaten');

            // Download file excel menggunakan nama kabupaten
            return Excel::download(new DataByKabExport($dataKab), 'test.xlsx');
        } catch (\Throwable $th) {
            return response()->json(['error' => 'Terjadi kesalahan saat mengekspor data', 'data' => $th->getMessage()], 500);
        }
    }
